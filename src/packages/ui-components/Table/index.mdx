...
Você pode de uma maneira fácil utizar o hook **usePaginatedObservable** do **@eduzz/houston-hooks**:
...
<Playground>
  {() => {
    const { error, isLoading, result, retry } = usePaginatedObservable(
...
| mobileWidth       | `number      | false`      | `600`    | Tamanho da tela que a tabela irá para o modo mobile, se false a tabela permanecerá sempre uma table |
...
| idnex | `numer` | `true`      | -      | Index da linha, é necessário para o componente `Table.Action`.   |
...
| mobileSize  | `GridSize     | `false`     | `12`            | Grid system que cada celula irá seguir no modo mobile                                        |
